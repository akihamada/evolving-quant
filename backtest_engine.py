...
使用:
    python backtest_engine.py --years 30
    # または import 経由で
...
# 時間減衰: 10年半減期で古いサンプルを「歴史の教訓」として尊重しつつ
# 直近をやや重視する（バフェット哲学: 30年の歴史から学ぶが、現代も無視しない）
# 30年前 → 0.125 / 20年前 → 0.25 / 10年前 → 0.5 / 5年前 → 0.71 / 直近 → 1.0
TIME_DECAY_HALFLIFE_YEARS = 10.0

# 短期間銘柄に対するセクター proxy 設定
# 銘柄の歴史が浅い場合、これらの代替ETFで擬似的に古いデータを補う
SECTOR_PROXY_MAP: dict[str, str] = {
    # AI/Semi 系の古い銘柄
    "PLTR":  "QQQ",   # Tech 全般 ETF
    "ARM":   "SOXX",  # Semi ETF
    "OKLO":  "XLU",   # 公益 ETF (原子力)
    "NNE":   "XLU",
    "DDOG":  "QQQ",
...
def _time_decay_weight(days_back: int, halflife_years: float = TIME_DECAY_HALFLIFE_YEARS) -> float:
    """指数時間減衰の重み。

    days_back=0 (今日) → 1.0
    days_back=halflife*252 → 0.5
    days_back=halflife*252*4 → 0.0625

    Args:
        days_back: 現在からの日数（正値）
        halflife_years: 半減期年数
    ...
    if days_back <= 0:
        return 1.0
    halflife_days = halflife_years * 252
    return float(np.exp(-np.log(2) * days_back / halflife_days))


def _detect_max_history(prices: np.ndarray, years_max: int = 30) -> int:
    """利用可能な最大履歴年数を返す。"""
    if prices is None:
        return 0
    n_days = len(prices)
    return min(years_max, n_days // 252)
...
def run_full_backtest(
    ...
    # 集計バケット — 時間減衰重み付き
    aggregate = {
        h: {name: {"weighted_hits": 0.0, "weighted_trials": 0.0,
                   "raw_hits": 0, "raw_trials": 0}
    ...
    logger.info(
        "🚀 バックテスト学習開始: %d銘柄 × 最大%d年 / sampling=%d日毎 / 時間減衰半減期=%.0f年",
        len(tickers), years, sample_every_days, TIME_DECAY_HALFLIFE_YEARS,
    )
    if years >= 20:
        logger.warning(
            "⚠️ 30年級バックテスト: 古い銘柄ほど生存バイアスに注意。"
            "倒産企業が含まれない楽観的傾向あり。時間減衰で直近を重視。"
        )

    for ticker in tickers:
        prices = prices_dict.get(ticker)
        ticker_history_years = _detect_max_history(prices, years)

        # 短期間銘柄: セクタープロキシで補完
        proxy_used = None
        if ticker_history_years < 5 and SECTOR_PROXY_MAP.get(ticker):
            proxy_ticker = SECTOR_PROXY_MAP[ticker]
            proxy_prices = prices_dict.get(proxy_ticker)
            if proxy_prices is not None and len(proxy_prices) >= 252 * 5:
                # プロキシ価格を「銘柄の延長」として使用
                # 銘柄実履歴の最古値で正規化してプロキシを接続
                if prices is not None and len(prices) > 0:
                    scale = float(prices[0]) / float(proxy_prices[-len(prices)]) \
                            if len(proxy_prices) >= len(prices) else 1.0
                    extended_proxy = np.array(proxy_prices) * scale
                    # 結合: 古い proxy + 新しい実価格
                    n_proxy = len(extended_proxy) - len(prices)
                    if n_proxy > 0:
                        prices = np.concatenate([extended_proxy[:n_proxy], prices])
                        n_with_proxy += 1
                        proxy_used = proxy_ticker
                        logger.info(
                            "  🔄 %s: %s をproxyとして接続 (実履歴 %.1f年 → 拡張 %.1f年)",
                            ticker, proxy_ticker,
                            ticker_history_years, _detect_max_history(prices, years)
                        )

        if prices is None or len(prices) < 100:
            logger.warning("  %s: データ不足 (%d日) — スキップ", ticker, len(prices) if prices is not None else 0)
            continue
        ...
        for idx in range(start_idx, end_idx, sample_every_days):
            # 時間減衰重み (idx は負値なので絶対値が日数)
            days_back = abs(idx)
            decay_w = _time_decay_weight(days_back)
            ...
            for h in eval_horizons:
                ...
                # 各ファクターの方向と実方向の一致を時間重み付きでカウント
                for name, score in result["factor_scores"].items():
                    pred_dir = _safe_signum(score)
                    if pred_dir == 0:
                        continue
                    aggregate[h][name]["raw_trials"] += 1
                    aggregate[h][name]["weighted_trials"] += decay_w
                    if pred_dir == actual_dir:
                        aggregate[h][name]["raw_hits"] += 1
                        aggregate[h][name]["weighted_hits"] += decay_w
        ...
        by_ticker_stats[ticker] = {
            ...
            "history_years_actual": ticker_history_years,
        ...
        proxy_note = f" + proxy={proxy_used}" if proxy_used else ""
        logger.info("  ✅ %s: %d サンプル (%.1f年%s)",
                    ticker, ticker_samples, ticker_history_years, proxy_note)

    # 集計を精度に変換 (時間重み付き)
    factor_stats: dict[str, dict[str, Any]] = {}
    for name in FACTOR_NAMES:
        h7 = aggregate.get(7, {}).get(name,
            {"weighted_hits": 0, "weighted_trials": 0, "raw_hits": 0, "raw_trials": 0})
        h30 = aggregate.get(30, {}).get(name,
            {"weighted_hits": 0, "weighted_trials": 0, "raw_hits": 0, "raw_trials": 0})

        # 時間減衰重み付き精度（直近重視）
        acc7 = h7["weighted_hits"] / h7["weighted_trials"] if h7["weighted_trials"] else 0.5
        acc30 = h30["weighted_hits"] / h30["weighted_trials"] if h30["weighted_trials"] else 0.5
        total_w = h7["weighted_trials"] + h30["weighted_trials"]
        weighted_acc = (
            (acc7 * h7["weighted_trials"] + acc30 * h30["weighted_trials"]) / total_w
            if total_w else 0.5
        )
        # raw 精度（参考: 全期間平均）
        raw7 = h7["raw_hits"] / h7["raw_trials"] if h7["raw_trials"] else 0.5
        raw30 = h30["raw_hits"] / h30["raw_trials"] if h30["raw_trials"] else 0.5

        factor_stats[name] = {
            "accuracy_7d": round(acc7, 4),
            "accuracy_30d": round(acc30, 4),
            "accuracy": round(weighted_acc, 4),
            "raw_accuracy_7d": round(raw7, 4),
            "raw_accuracy_30d": round(raw30, 4),
            "samples_7d": int(h7["raw_trials"]),
            "samples_30d": int(h30["raw_trials"]),
            "samples": int(h7["raw_trials"] + h30["raw_trials"]),
    ...
    return {
        ...
        "time_decay_halflife_years": TIME_DECAY_HALFLIFE_YEARS,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Master Wisdom 30年バックテスト学習")
    parser.add_argument("--years", type=int, default=30,
                        help="バックテスト年数 (デフォルト30 / 銘柄ごとに最大利用可能期間)")
    parser.add_argument("--sample-days", type=int, default=5,
                        help="サンプリング間隔(日)、5=週次")
    parser.add_argument("--no-proxy", action="store_true",
                        help="セクタープロキシで短期間銘柄を補完しない")
    ...
    # プロキシ用ティッカーも取得
    proxy_tickers = list(set(SECTOR_PROXY_MAP.values())) if not args.no_proxy else []
    all_tickers_to_fetch = list(set(tickers + proxy_tickers))

    # 価格履歴 - max を指定して取得（個別銘柄の上限に従う）
    period_str = "max"  # yfinance は銘柄ごとに利用可能な最大期間を返す
    logger.info(
        "📥 価格データ取得 (max): %d銘柄 + proxy %d銘柄",
        len(tickers), len(proxy_tickers),
    )
    data = yf.download(all_tickers_to_fetch, period=period_str, progress=False)
    ...
    for t in all_tickers_to_fetch:
        if t in prices_close.columns:
            ...
            if len(arr) >= 200:
                prices_dict[t] = arr
                years_avail = len(arr) / 252
                logger.info("  📊 %s: %.1f 年分", t, years_avail)
    ...
    # マクロ (VIX は 1990年〜, SP500 は1928年〜利用可能)
    macro_history: dict[str, np.ndarray] = {}
    try:
        ...
        if not vix.empty:
            ...
            macro_history["vix"] = vix_arr
            logger.info("  📈 ^VIX: %.1f 年分", len(vix_arr) / 252)
        ...
        if not spy.empty:
            ...
            macro_history["spy"] = spy_arr
            logger.info("  📈 ^GSPC: %.1f 年分", len(spy_arr) / 252)
